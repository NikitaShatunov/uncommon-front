      ...
      <div className={`cart ${isCartShown ? "cart__shown" : ""}`}>
        ...
        {!items.length && (
          <div className="cartempty">
            <div className="cartempty__caption">YOUR CART IS CURRENTLY EMPTY.</div>
           <div className="cartempty__button"> <Button
              available={true}
              size="medium"
              primary={true}
        ...
        {Boolean(items.length) && <div>
         <div className="cartList"> {items.map((item, id) => <CartItem key={id} {...item}/>)}</div>
         <div className="cartList__button">
          <div>
          <div className="totatlSum">subtotal: {sum}</div>
          <Button size="medium" primary={true} onClickButton={() => console.log('click')} available={true}>BUY NOW</Button></div>
          </div>
          </div>}
      </div>
      <div
